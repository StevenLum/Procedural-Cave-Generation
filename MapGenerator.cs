...
public class MapGenerator : MonoBehaviour {
	public enum CellularAutomata { DiagonalBias, Normal }
	...
	public void GenerateMap() {
		...
		for (int i = 0; i < smoothAmount; i++) {
			if (cellularAutomata == CellularAutomata.DiagonalBias)
				SmoothMapDiagonal();
			if (cellularAutomata == CellularAutomata.Normal)
				SmoothMapNormal();
		}
		...
		for (int x = 0; x < borderedMap.GetLength(0); x++) {
			for (int y = 0; y < borderedMap.GetLength(1); y++) {
				...
				if((x <= borderSize || x > width + borderSize) || (y <= borderSize || y > height + borderSize)) { 
					...
					//Debug.Log("else " + x + y);
					borderedMap[x, y] = map[x - borderSize-1, y - borderSize-1];
				}*/
			}
		}
